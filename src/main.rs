...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let matches = Command::new("Code Collater")
        .version("2.2")
        .author("Sathya Sheelan <sathya.sheelan@gmail.com>")
    ...
    debug!("Valid extensions: {:?}", valid_extensions);

    let skip_extensions: HashSet<String> = matches
        .get_many::<String>("skip")
        .map(|v| v.map(|s| s.clone()).collect())
        .unwrap_or_default();

    debug!("Skip extensions: {:?}", skip_extensions);

    let skip_folders: HashSet<String> = matches
        .get_many::<String>("skip-folders")
        .map(|v| v.map(|s| s.clone()).collect())
        .unwrap_or_default();
    ...
    let found_files = find_files(
        directory,
        &valid_extensions,
        &skip_extensions.iter().map(|s| s.as_str()).collect(),
        &skip_folders.iter().map(|s| s.as_str()).collect(),
    );

    if !found_files.is_empty() {
        // Process files in parallel
        let results: Vec<_> = found_files
            .into_par_iter()
        ...
        info!("Summary created: {}", matches.get_one::<String>("output").unwrap());
    } else {
